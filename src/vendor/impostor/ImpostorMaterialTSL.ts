...
/**
 * TSL node type alias + cast helper.
 *
 * The component-wise builders above (mul/pow/clamp/...) produce correct WGSL at
 * runtime, but @types/three 0.183.1's TSL overloads are narrower than the
 * actual node algebra (e.g. they over-constrain a vec3 result to vec4, or a
 * component-wise `pow(vec3, vec3)` to `pow(vec3, float)`). `asNode()` is a
 * typing-only escape hatch used at the handful of call sites where the static
 * overloads disagree with the (valid) shader graph. It changes NO runtime
 * behavior — it only relaxes the compile-time node-flavor check.
 */
type TSLNode = ReturnType<typeof THREE_NAMESPACE.TSL.float>;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const asNode = (n: unknown): TSLNode => n as any;
...
export type TSLImpostorMaterial = THREE_NAMESPACE.MeshBasicNodeMaterial & {
  ...
  updateLighting?: (config: TSLImpostorLightingUpdate) => void;
};

/** Lighting update payload accepted by {@link TSLImpostorMaterial.updateLighting}. */
export type TSLImpostorLightingUpdate = {
...
export function createTSLImpostorMaterial(
  ...
  if (debugMode === 6) {
    ...
    colorNode = Fn(() => {
      ...
      const useDepthBlendingBool = uUseDepthBlending.equal(int(1));
      const wa_raw = select(useDepthBlendingBool, wa_depth, wa_std);
      const wb_raw = select(useDepthBlendingBool, wb_depth, wb_std);
      const wc_raw = select(useDepthBlendingBool, wc_depth, wc_std);
      ...
      const albedoDecoded = pow(
        asNode(albedo.rgb),
        asNode(vec3(2.2, 2.2, 2.2)),
      );
      ...
      const usePBRBool = uUsePBR.equal(int(1));
      const roughness = select(usePBRBool, pbrBlended.r, float(0.8));
      const metallic = select(usePBRBool, pbrBlended.g, float(0));
      const ao = select(usePBRBool, pbrBlended.b, float(1));
      ...
      // =========================================================================
      const dirDiffuses: TSLNode[] = [];
      const dirSpeculars: TSLNode[] = [];
      ...
      // =========================================================================
      const ptDiffuses: TSLNode[] = [];
      const ptSpeculars: TSLNode[] = [];
      ...
      const litColor = add(
        diffuseContrib,
        mul(asNode(totalSpecular), float(0.5)),
      );
      ...
      const clampedColor = clamp(
        asNode(colorToOutput),
        asNode(vec3(0, 0, 0)),
        asNode(vec3(1, 1, 1)),
      );
      ...
      return vec4(asNode(clampedColor), totalWeight);
    })();
  }
  ...
  if (enableAAA) {
    tslMaterial.updateLighting = (config: TSLImpostorLightingUpdate) => {
